...
def get_ohlcv_kiwoom(code, start_date="20140101"):
    ...
    for _ in range(100):
        ...
        current_date = (oldest_date - timedelta(days=1)).strftime('%Y%m%d')
        time.sleep(1)
...
def analyze_e_book_signals(df):
    ...
    def classify(val):
        ...
        else: return f"중립({val:.1f}%)"

    result['이격도_20일'] = classify(d20)
    result['이격도_60일'] = classify(d60)
    ...
    t26_high = df['High'].rolling(26).max()
    t26_low = df['Low'].rolling(26).min()
    df['기준선'] = (t26_high + t26_low) / 2

    df['선행스팬1'] = ((df['전환선'] + df['기준선']) / 2).shift(26)
    t52_high = df['High'].rolling(52).max()
    t52_low = df['Low'].rolling(52).min()
    df['선행스팬2'] = ((t52_high + t52_low) / 2).shift(26)
...
def analyze_stock(symbol):
    ...
    e_book = analyze_e_book_signals(df)
    cur_price = df["Close"].iloc[-1]
    future = {}
    change = {}

    for d in [1, 5, 10, 20, 40, 60, 80]:
        pred = round(cur_price * (1 + 0.002 * d), 2)
        future[f"{d}일"] = pred
        change[f"{d}일"] = round((pred - cur_price) / cur_price * 100, 2)
...
@app.route("/")
def index():
    return "📈 Kiwoom + 일목균형표 분석 API 가동 중"
