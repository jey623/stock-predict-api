...
# 📈 키움 API 일봉 조회
def get_ohlcv_kiwoom(code: str, base_dt: str = None):
    ...
    res = requests.post(url, headers=headers, json=data)
    if res.status_code != 200:
        raise Exception(f"❌ 일봉 데이터 요청 실패: {res.status_code} {res.text}")
    output = res.json().get("output1", [])
    if not output:
        raise Exception("❌ 조회된 일봉 데이터가 없습니다.")
...
@app.route("/")
def home():
    return "📈 Kiwoom REST API with Ichimoku Analysis is running."
